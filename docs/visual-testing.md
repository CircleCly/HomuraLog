...
13. compact view after an injected pointer click on the right branch arrow;
14. compact view after an injected pointer click on the down arrow;
15. compact view after injected wheel zoom and pointer drag.
16. large view after an injected node-row click, wheel zoom, and canvas drag.
...
In addition to screenshot state, the suite asserts that opening the large view hides the compact window, preserves and precisely centers the compact view's focused node, closing it restores the compact window, the compact canvas retains its local navigation safe area and readable automatic zoom, and the large view hides GraphEdit's built-in toolbar in favor of localized controls.
...
The suite injects Godot pointer events for compact branch arrows, wheel zoom, canvas dragging, node rows, and jump/delete buttons, and records state-change assertions in `HomuraLog.log`. Jump/delete requests are intercepted immediately before their mutating side effects, so the user's combat and timeline data are not changed. Manual checks are still required for subjective drag/zoom feel and an opt-in end-to-end replay execution pass.
